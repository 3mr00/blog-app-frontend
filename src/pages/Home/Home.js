...
export default function Home(props) {
  ...
  const loadBlogsData = async () => {
    ...
    const response = await fetch(
      `https://blog-app-backend-amr.herokuapp.com/blogs?_page=1&_limit=${limit}`
    );
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure that you wanted to delet that blog ?")) {
      const response = await axios.delete(
        `https://blog-app-backend-amr.herokuapp.com/blogs/${id}`
      );
      if (response.status === 200) {
  ...
  const handleCategory = async (category) => {
    const response = await fetch(
      `https://blog-app-backend-amr.herokuapp.com/blogs?category=${category}`
    );
    const data = await response.json();
  ...
  const fetchPageClick = async (currentPage) => {
    const res = await fetch(
      `https://blog-app-backend-amr.herokuapp.com/blogs?_page=${currentPage}&_limit=${limit}`
    );
  ...
  const fetchsetRecomenededPost = async () => {
    ...
    const response = await axios.get(
      `https://blog-app-backend-amr.herokuapp.com/blogs?per_page=4&page=${page}`
    );
    if (response.status === 200) {
      setRecomenededPost(
        response.data
